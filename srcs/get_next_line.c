...
/*   Updated: 2021/03/19 19:56:44 by budal-bi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

char	*ft_run(char *reste, char *buf, char **line)
{
	int		bs;
	char	*temp;
	char	*sub;

	bs = ft_chrlen_gnl(buf, '\n');
	free(reste);
	if (bs == 0)
	{
		reste = ft_substr_gnl(buf, bs + 1, (ft_chrlen_gnl(&buf[bs + 1], '\0')));
		if (!(*line = malloc(1)))
			return (NULL);
		*line[0] = '\0';
	}
	else if (bs != -1 && bs != 0)
	{
		...
		sub = ft_substr_gnl(buf, 0, bs);
		*line = ft_strjoin_gnl(temp, sub);
		free(temp);
		free(sub);
	}
	else
		reste = ft_strdup_gnl(buf);
	return (reste);
}
...
char	*ft_run_end(char *r, char **line)
{
	int		bs;
	char	*temp;
	...
	if (bs == 0)
	{
		temp = ft_substr_gnl(r, bs + 1, (ft_chrlen_gnl(&r[bs + 1], '\0')));
		free(r);
		r = ft_strdup_gnl(temp);
		*line = malloc(1);
		*line[0] = '\0';
		free(temp);
	}
	else if (bs != -1 && bs != 0)
		r = ulti_run(r, line, bs);
	else
	{
		temp = ft_strdup_gnl(*line);
		free(*line);
		*line = ft_strjoin_gnl(temp, r);
		free(r);
		r = NULL;
		free(temp);
	}
